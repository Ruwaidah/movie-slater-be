            ...
            .then(response => {
                movieById(req, 3, 4)
                    .then(response2 => {
                        ...
                        let movie1 = response.data.filter(movie => req.body.movies.includes(movie.tmsId))
                        let movie2 = response2.data.filter(movie => req.body.movies.includes(movie.tmsId))
...
function movieById(req, days, number) {
    return axios.get(
        `http://data.tmsapi.com/v1.1/movies/showings?startDate=${getday(days)}&zip=${checkZip(req)}&radius=10&numDays=${number}&api_key=${process.env.API_KEY}`)
}
