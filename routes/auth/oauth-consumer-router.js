          ...
          if (!consum) {
            Consumer.add(
              ...
              .then((resp) =>
                res
                  .status(201)
                  .json({ token: signToken(response.data), user: resp })
              )
              .catch((error) =>
                res.status(500).json({ message: "error adding data" })
              );
          } else
            res.status(200).json({ token: signToken(consum), user: consum });
        })
        .catch((error) =>
...
router.put("/:googleId", restricted, (req, res) => {
  Consumer.updateUser(req.body, "oauth_consumer", {
    googleId: req.params.googleId,
  })
    .then((user) => {
      if (user) {
        Consumer.findBytheater({ user_id: user.id }).then((theatres) => {
          res.status(200).json({
            user: {
              id: user.id,
              googleId: user.googleId,
